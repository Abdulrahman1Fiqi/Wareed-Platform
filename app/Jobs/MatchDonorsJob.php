...
class MatchDonorsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $existingDonorIds = DonorResponse::where('blood_request_id', $this->bloodRequest->id)
            ->pluck('donor_id')
            ->toArray();
        ...
        $donors = User::where('role', 'donor')
            ...
            ->where(function ($query) use ($compatibleTypes) {
            ...
            ->when(!empty($existingDonorIds), function ($query) use ($existingDonorIds) {
                $query->whereNotIn('id', $existingDonorIds);
            })
        ...
        if ($donors->isEmpty()) {
            \Log::info('MatchDonorsJob: no matching donors', [
                'hospital_city' => $hospital->city,
                'blood_type'    => $this->bloodRequest->blood_type,
            ]);
            return;
        ...
        foreach ($donors as $donor) {
            try {
                ...
                \Log::info('MatchDonorsJob: donor notified', [
                    'donor_id'   => $donor->id,
                    'donor_name' => $donor->name,
                ]);
            } catch (\Exception $e) {
                \Log::error('MatchDonorsJob: failed to notify donor', [
                    'donor_id' => $donor->id,
                    'error'    => $e->getMessage(),
                ]);
            }
